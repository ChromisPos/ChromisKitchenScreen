...
public class DataLogicKitchen {
    ...
    public void createOrder(Orders orderData) {
        ...
        sql_query = "INSERT INTO ORDERS (ID, ORDERID, QTY, DETAILS, ATTRIBUTES, NOTES, TICKETID, ORDERTIME, DISPLAYID, AUXILIARY, SEQUENCE, KITCHENTIME)"
                + " VALUES ( :id, :orderid, :qty, :details, :attributes, :notes, :ticketid, :ordertime, :displayid, :auxiliaryid, :sequenceid, :kitchentimeid )";
        init();
        session.beginTransaction();
        query = HibernateUtil.getSessionFactory().openSession().createSQLQuery(sql_query);
        ...
        query.setParameter("auxiliaryid", orderData.getAuxiliaryid());
        query.setParameter("sequenceid", orderData.getSequenceid());
        query.setParameter("kitchentimeid", orderData.getKitchenTime());
        int result = query.executeUpdate();
        session.getTransaction().commit();
        session.close();
    }
